...
async function initDB() {
  ...
  // V7.7.42: one authoritative access snapshot. Registration/hierarchy is used only until Super Admin sets this row.
  await pool.query(`CREATE TABLE IF NOT EXISTS user_effective_access(
    employee_number TEXT PRIMARY KEY,
    base_profile TEXT NOT NULL CHECK (base_profile IN ('VIEW','EDIT','FULL_ACCESS')),
...
function designationBand(designation='') {
  const d = normalizedDesignation(designation);
  if (['chairman cum managing director','cmd','director'].includes(d)) return 'TOP_MANAGEMENT';
  if (['executive director','ed','general manager','gm','chief general manager','cgm'].includes(d)) return 'SENIOR_MANAGEMENT';
...
function inheritedPermissionCodes({designation='',operationalRole='NORMAL_USER',responsibilities=[]}={}){
  ...
  // V7.7.42 USER GOVERNANCE BASELINE
  // RBAC + least-privilege: registration establishes only the minimum job-function baseline.
  // A later Super Admin manual override is authoritative and replaces this baseline immediately.
  ...
  const isHod = op==='HOD' || roles.has('HOD');
  const isSuper = op==='SUPER_ADMIN' || roles.has('SUPER_ADMIN') || roles.has('OWNER');

  // DGM and above: management-grade access in assigned organisational scope.
  if(['DGM','SENIOR_MANAGEMENT','TOP_MANAGEMENT'].includes(band)){
    ['FULL_ACCESS','DELETE_UNDO','APPROVAL','PRINT_EXPORT','ANALYSIS','REPORTS','ADVANCED_REPORTS','RCM'].forEach(x=>out.add(x));
  }
  // HOD: full maintenance decision/reporting capability in HOD scope; master/access administration remains separate.
  if(isHod){
    ['FULL_ACCESS','DELETE_UNDO','APPROVAL','PRINT_EXPORT','ANALYSIS','REPORTS','ADVANCED_REPORTS','RCM','MASTER_EDIT'].forEach(x=>out.add(x));
  }
  // Super Admin/Owner: plant-wide security/access administration in addition to maintenance capabilities.
  if(isSuper){
    ['FULL_ACCESS','DELETE_UNDO','APPROVAL','PRINT_EXPORT','ANALYSIS','REPORTS','ADVANCED_REPORTS','RCM','MASTER_EDIT','ACCESS_ADMIN'].forEach(x=>out.add(x));
  }
  return [...out];
}
...
async function effectiveAuthority(employeeNumber) {
  ...
  // V7.7.33: manual access override is authoritative for BOTH runtime permissions and admin summary.
  // Registration/hierarchy remains the baseline only when no override exists.
  if (hasAccessOverride) {
    const os = new Set(overridePermissions);
    if (os.has('FULL_ACCESS')) effectiveAccess = 'FULL';
    else if (os.has('EDIT') || os.has('ENTRY')) effectiveAccess = 'ENTRY_VIEW';
    else if (os.has('VIEW')) effectiveAccess = 'VIEW';
    else effectiveAccess = 'NONE';
  }
...
async function syncBundledLmmmKnowledge(){
  // V7.7.65: bundled JSON is authoritative bootstrap data. Rebuild the two
  // bundled tables from the package whenever package version/count changes.
  // Live WhatsApp/event tables are separate and are never deleted here.
  try{
    const core=JSON.parse(await fs.readFile(new URL('./data/master_core_records.json',import.meta.url),'utf8'));
    const unified=JSON.parse(await fs.readFile(new URL('./data/unified_retrieval_records.json',import.meta.url),'utf8'));
    if(!Array.isArray(core) || !Array.isArray(unified)) throw new Error('V7.7.65 package data must be compatibility arrays');
    const key=`v7765:${core.length}:${unified.length}`;
    const done=(await pool.query(`SELECT 1 FROM lmmm_master_sync_meta WHERE sync_key=$1 LIMIT 1`,[key])).rows.length>0;
    if(done){console.log('[V7.7.65 MASTER SYNC] already current',key);return;}
    await pool.query('BEGIN');
    ...
    await pool.query(`INSERT INTO lmmm_master_records(uid,record_type,equipment,area,event_date,record_text,source_name,source_payload)
      ...
      FROM jsonb_to_recordset($1::jsonb) AS x(uid text,record_type text,equipment text,area text,event_date text,text text,source text,payload jsonb)`,[JSON.stringify(core)]);
    await pool.query(`DELETE FROM lmmm_knowledge_records`);
    await pool.query(`INSERT INTO lmmm_knowledge_records(uid,source_name,source_row,entity_types,raw_text,normalized_text,identifiers,verification_status,source_payload)
      ...
      FROM jsonb_to_recordset($1::jsonb) AS x(uid text,source_file text,source_row text,entity_types text[],raw_text text,normalized_text text,identifiers text[],verification_status text)`,[JSON.stringify(unified)]);
    await pool.query(`DELETE FROM lmmm_master_sync_meta`);
    await pool.query(`INSERT INTO lmmm_master_sync_meta(sync_key,record_count,synced_at) VALUES($1,$2,now())`,[key,core.length+unified.length]);
    await pool.query('COMMIT');
    console.log('[V7.7.65 MASTER SYNC]',JSON.stringify({core:core.length,unified:unified.length,key}));
  }catch(e){try{await pool.query('ROLLBACK')}catch{} console.error('[V7.7.65 MASTER SYNC ERROR]',e);}
}
...
async function authoritativeAccessState(employeeNumber){
 const emp=String(employeeNumber||'').trim(); if(!emp)return null;
 const q=await pool.query(`SELECT base_profile,extras,updated_by,updated_at FROM user_effective_access WHERE employee_number=$1 LIMIT 1`,[emp]);
 if(q.rows[0]){
   const r=q.rows[0], extras=Array.isArray(r.extras)?r.extras.map(x=>String(x).toUpperCase()):[];
   ...
   return {manual:true,base,codes:[...codes],updated_by:r.updated_by,updated_at:r.updated_at};
 }
 // One-time compatibility migration from the older override table.
 const old=(await pool.query(`SELECT enabled,permissions,updated_by,updated_at FROM user_access_override WHERE employee_number=$1 LIMIT 1`,[emp])).rows[0];
 if(old?.enabled===true){
   const arr=Array.isArray(old.permissions)?old.permissions.map(x=>String(x).toUpperCase()):[];
   const set=new Set(arr); let base=set.has('FULL_ACCESS')?'FULL_ACCESS':(set.has('EDIT')||set.has('ENTRY')?'EDIT':'VIEW');
   const extras=arr.filter(x=>!['FULL_ACCESS','VIEW','EDIT','ENTRY'].includes(x));
   await pool.query(`INSERT INTO user_effective_access(employee_number,base_profile,extras,updated_by,updated_at,reason) VALUES($1,$2,$3::jsonb,$4,now(),'Migrated from legacy override') ON CONFLICT(employee_number) DO NOTHING`,[emp,base,JSON.stringify(extras),old.updated_by||'SYSTEM']);
   ...
   return {manual:true,base,codes:[...codes],updated_by:old.updated_by,updated_at:old.updated_at};
 }
 return null;
}
async function saveAuthoritativeAccess(employeeNumber,baseProfile,extras,updatedBy,reason){
 const emp=String(employeeNumber||'').trim(), base=String(baseProfile||'VIEW').toUpperCase();
 if(!['VIEW','EDIT','FULL_ACCESS'].includes(base))throw new Error('Invalid base access profile');
 const clean=[...new Set((extras||[]).map(x=>String(x).toUpperCase()).filter(x=>!['FULL_ACCESS','VIEW','EDIT','ENTRY'].includes(x)))];
 // FULL_ACCESS is intentionally represented only by base_profile; stale grants cannot resurrect it.
...
async function sendAccessChangeFeedback(adminWa,emp){
...
async function universalSearch(q,u){
 ...
 // V7.7.65 area-level module routing: "BDM history" is an area request, not an equipment name.
 const areaToken=String(entity||'').trim().toUpperCase().replace(/[-_]+/g,' ');
 if(['BDM','BAR MILL','FINISHING'].includes(areaToken) && ['history','defect','job_action'].includes(intent)){
   const canonicalArea=areaToken==='BAR MILL'?'Bar Mill':areaToken==='FINISHING'?'Finishing':'BDM';
   const sc=await effectiveSearchScope(u);
   if(!areaMatchesScope(canonicalArea,sc,'')) return {text:'This equipment/area is outside your authorised work scope.',status:'OUT_OF_SCOPE'};
   const vals=[canonicalArea]; let typeWhere='';
   if(intent==='history'||intent==='job_action'){vals.push('history');typeWhere=` AND record_type=$2`;}
   if(intent==='defect'){vals.push('defect');typeWhere=` AND record_type=$2`;}
   const rows=(await pool.query(`SELECT uid,record_type,equipment,area,event_date,record_text,source_name,source_payload FROM lmmm_master_records WHERE UPPER(COALESCE(area,''))=UPPER($1)${typeWhere} ORDER BY CASE WHEN event_date ~ '^\d{4}-\d{2}-\d{2}$' THEN event_date::date END DESC NULLS LAST,uid LIMIT 30`,vals)).rows;
   const filtered=intent==='job_action'?rows.filter(isJobHistoryRow):rows;
   if(!filtered.length)return {text:`${canonicalArea} — No source-backed ${intent==='defect'?'defect':intent==='history'?'history':'job'} records found in the currently indexed data.`,buttons:[],status:'NO_MODULE_DATA'};
   return {text:`${canonicalArea} — ${intent==='defect'?'Defects':intent==='history'?'History':'Jobs'}\nShowing latest ${Math.min(filtered.length,20)}\n\n${formatBundledResults(filtered.slice(0,20))}`,buttons:[],status:'OK'};
 }
...
async function sendAccessAdminMenu(to,emp){
 ...
 await syncHierarchyAndAccess(emp,to,'Access screen role self-heal').catch(e=>console.error('[ACCESS SELF HEAL]',e.message));
...
async function sendPermissionAdmin(to,emp,page=1){
 ...
 const __snapPick=await authoritativeAccessState(emp); const active=new Set((__snapPick?.manual?__snapPick.codes:((a.has_access_override?a.override_permissions:a.special_permissions)||[])).map(x=>String(x).toUpperCase()));
...
async function applyAccessSelection(from,emp){
 ...
 const base=sel.has('FULL_ACCESS')?'FULL_ACCESS':(sel.has('EDIT')||sel.has('ENTRY')?'EDIT':'VIEW');
 const extras=[...sel].filter(x=>!['FULL_ACCESS','VIEW','EDIT','ENTRY'].includes(x));
 await saveAuthoritativeAccess(emp,base,extras,from,'Super Admin Apply Selection V7.7.42');
...
async function ownerCommand(from, text) {
  ...
  // V7.7.65 fallback for WhatsApp clients that return only the visible quick-reply title.
  if(/^Remove$/i.test(String(text||'').trim())){
    const st=await selectionGet(from,'PENDING_ADMIN_REMOVE','',[]);
    const emp=String(st.selected?.[0]||'');
    if(/^\d+$/.test(emp)){
      const removed=await removeUserRegistrationForReregister(emp,from);
      await selectionClear(from,'PENDING_ADMIN_REMOVE','');
      if(removed){await sendText(removed.whatsapp_number,'Your LMMM AI Maintenance registration has been removed. Send Hi to register again.');await sendText(from,`${emp} registration removed. Historical maintenance records preserved.`);}
      else await sendText(from,'Not found.');
      return true;
    }
  }
  ...
  if (m) {
    const removed = await removeUserRegistrationForReregister(m[1],from);
    await selectionClear(from,'PENDING_ADMIN_REMOVE','').catch(()=>{});
    if (!removed) {
      await sendText(from, 'Not found.');
      return true;
    }
