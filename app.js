...
const OWNER_NUMBERS = new Set(
  (process.env.OWNER_NUMBERS || process.env.OWNER_NUMBER || process.env.SUPER_ADMIN_NUMBER || '')
...
async function initDB(){
 ...
 console.log('[DATABASE] V4 approval-fix ready');
}
...
async function adminNumbers(){
 const nums=new Set(OWNER_NUMBERS);
 if(pool){
  try{
   const r=await pool.query(`SELECT whatsapp_number FROM users WHERE is_active=true AND lower(coalesce(approval_status,''))='approved' AND (lower(coalesce(designation,'')) IN ('super admin','owner','administrator','admin') OR lower(coalesce(responsibility,'')) IN ('super admin','owner','administrator','admin'))`);
   for(const row of r.rows){const n=String(row.whatsapp_number||'').replace(/\D/g,'');if(n)nums.add(n);}
  }catch(e){console.error('[ADMIN LOOKUP ERROR]',e);}
 }
 return [...nums];
}
async function isAdmin(from){
 const n=String(from||'').replace(/\D/g,'');
 return (await adminNumbers()).includes(n);
}
async function notifyAdmins(d){
 const admins=await adminNumbers();
 console.log('[APPROVAL] Admin count:',admins.length);
 if(!admins.length){
  console.error('[APPROVAL] NO SUPER ADMIN CONFIGURED. Set SUPER_ADMIN_NUMBER or OWNER_NUMBER(S) in Render.');
  return false;
 }
 let sent=0;
 for(const owner of admins){
  try{
   await sendText(owner,`Registration approval:\n${d.name} / ${d.employee_number} / ${d.designation} / ${d.section} / ${d.area}\n\nAPPROVE ${d.employee_number}\nREJECT ${d.employee_number}`);
   sent++;
   console.log('[APPROVAL] Sent to admin',owner);
  }catch(e){console.error('[APPROVAL SEND ERROR]',owner,e);}
 }
 return sent>0;
}

async function ownerCommand(from,text){
 if(!(await isAdmin(from)))return false;
...
async function processMessage(from,text){
 ...
 if(!u){
  ...
  if(!notified) console.error('[REGISTRATION] Saved pending but admin notification failed for',d.employee_number);
  return;
 }
...
app.get('/health',async(_q,r)=>r.status(200).json({ok:true,database_configured:Boolean(DATABASE_URL),phone_number_id_configured:Boolean(PHONE_NUMBER_ID),access_token_configured:Boolean(ACCESS_TOKEN),super_admins_configured:(await adminNumbers()).length}));
...
app.get('/api/status',async(_q,r)=>r.status(200).json({status:'ready',registration:'V4-admin-fix',webhook:'/webhook',super_admins_configured:(await adminNumbers()).length}));
